...
function findClosestIntersection (cable1, cable2) {
  ...
  for (let i = 0; i < cable1.length; i++) {
    ...
    } else if (cable1[i].includes('D')) {
      cable1Position[1] -= parseInt(cable1[i].slice(1));
      if (cable2[i].includes('L')) {
        cable2Position[0] -= parseInt(cable2[i].slice(1));
        if (cable2Position[1] < cable1Position[0] < oldCable2Position[1]) {
          itersections.push(cable1Position[0] + cable2Position[1])
        }
      } else if (cable2[i].includes('R')) {
        cable2Position[0] += parseInt(cable2[i].slice(1));
        if (cable2Position[1] > cable1Position[0] > oldCable2Position[1]) {
          itersections.push(cable1Position[0] + cable2Position[1])
        }
      }
    } else if (cable1[i].includes('U')) {
      cable1Position[1] += parseInt(cable1[i].slice(1));
      if (cable2[i].includes('L')) {
        cable2Position[0] -= parseInt(cable2[i].slice(1));
        if (cable2Position[1] < cable1Position[0] < oldCable2Position[1]) {
    ...
    // console.log('cable1Position: ', cable1Position);
    // console.log('cable2Position: ', cable2Position);
    // if one of the values when it moves is in between the before and after of the other then they cross
  }
...
// need to see if when it goes across whether that number will coincide with the other one going down and visa versa

const simple1 = ['R1003','D138'];
const simple2 = ['L1003','U138'];


console.log(findClosestIntersection(cable1Movements, cable2Movements));
